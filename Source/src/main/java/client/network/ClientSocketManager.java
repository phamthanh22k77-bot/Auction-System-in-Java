...
public class ClientSocketManager {
    ...
    public void connect(String ip, int port) throws IOException {
        ...
        out.flush(); // gửi OOS header ngay để tránh deadlock với ObjectInputStream bên server

        in = new ObjectInputStream(socket.getInputStream());

        // Tiêu thụ WELCOME_MESSAGE server gửi ngay khi accept thành công
        try {
            PacketMessage welcomeMsg = (PacketMessage) in.readObject();
            if (welcomeMsg != null && welcomeMsg.getType() == MessageType.WELCOME_MESSAGE) {
                System.out.println("[ClientSocketManager] Kết nối tới Server thành công. Đã nhận: " + welcomeMsg.getType());
            }
